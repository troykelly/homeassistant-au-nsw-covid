...
async def reverse_geocode(case):
    global __GEO_CACHE

    if not (case.latitude and case.longitude):
        return None

    try:
        case_point = Point(case.latitude, case.longitude)
    except:
        case_point = None
    ...
    point_id = " ".join(map(str, case_point.coords[:]))
